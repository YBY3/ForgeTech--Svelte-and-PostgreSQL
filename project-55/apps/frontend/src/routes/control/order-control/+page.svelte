<script lang="ts">
	...
	onMount(() => {
		userData = data.user;
		// Sorting orders from earliest (oldest) to latest using the created_at property.
		unclaimedOrders = data.unclaimedOrders.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
		claimedOrders = data.claimedOrders.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
	});
				...
				<ul class="space-y-4">
					{#each unclaimedOrders as order (order.id)}
						<li class="group border rounded-lg p-4 transition-transform duration-300 transform hover:scale-105 cursor-pointer">
							...
							<button on:click|stopPropagation={() => claimOrder(order.id)}
				...
				<ul class="space-y-4">
					{#each claimedOrders as order (order.id)}
						<li class="group border rounded-lg p-4 transition-transform duration-300 transform hover:scale-105 cursor-pointer">
							...
							<button on:click|stopPropagation={() => unclaimOrder(order.id)}
